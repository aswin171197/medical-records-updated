...
const Login = ({ onLogin }) => {
  ...
  useEffect(() => {
    const checkBackendStatus = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/test-token`, {
          timeout: 3000
        });
...
const sendOtp = async () => {
  try {
    ...
    const response = await axios.post(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/send-otp-login`, {
      mobile: mobile
    });
...
const performPasswordLogin = async () => {
  ...
  try {
    ...
    const response = await axios.post(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/login`, loginData);
    ...
    if (error.code === 'ERR_NETWORK' || error.message === 'Network Error') {
      throw new Error(`Cannot connect to server. Please ensure the backend is running on ${process.env.REACT_APP_API_URL || 'https://medical-records-updates2.onrender.com/'}/`);
...
const handleOtpVerify = async (otp) => {
  ...
  try {
    const mobile = countryCode + formData.emailOrMobile.replace(/\D/g, ''); // Combine country code with mobile
    const response = await axios.post(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/verify-otp-login`, {
      mobile: mobile,
      otp: otp
    });
