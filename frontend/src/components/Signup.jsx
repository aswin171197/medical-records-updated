...
const Signup = ({ onSignup }) => {
  ...
  const performPasswordSignup = async () => {
    const response = await axios.post(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/signup`, {
      name: formData.name,
  ...
  const handleOtpVerify = async (otp) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.REACT_APP_API_URL?.replace(/\/$/, '') || 'https://medical-records-updates2.onrender.com/'}/auth/verify-otp-signup`, {
        mobile: mobile,
        otp: otp
      });
